...
if st.button("تأكيد الموقع"):
    ...
    # Load services and apartments data
    ...
    selected_services = [icon_mapping[feat] for feat in selected_features]
    filtered_services = df_services[df_services["Type_of_Utility"].isin(selected_services)]

    # Build KDTree for apartments
    shapes_tree = cKDTree(df_apartments[["latitude", "longitude"]].values)

    # Search radius (approximately 1 km)
    radius = 0.01

    # Find nearby apartments for each service
    nearest_indices = shapes_tree.query_ball_point(filtered_services[["Latitude", "Longitude"]].values, r=radius)

    # Extract nearby apartments
    ...
    # Plot nearby apartments on the map
